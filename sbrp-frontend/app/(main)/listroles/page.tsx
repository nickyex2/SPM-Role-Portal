...
export default function Test() {
	...
	function handleSearch(searchInput: string) {
		...
		if (searchBy === "Roles") {
			...
			if (search === "") {
				...
				let filteredRoles: TRoleListing[] = initialRoles.filter(
					(role) => {
	...
	async function createListing(
		...
		if (response.data.code === 201) {
			...
			let role_ids: Array<Number> = [
				response.data.data.role_id as number,
			];
	...
	useEffect(() => {
		...
		setCurrUserSkills(
			JSON.parse(sessionStorage.getItem("skills") as string)
		);
