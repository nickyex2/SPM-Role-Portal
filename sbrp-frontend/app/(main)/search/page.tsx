...
export default function SearchStaff() {
	...
	const [selected, setSelected] = useState<TRoleDetails | undefined>(
		undefined
	);
	...
	function searchRoles(query: string) {
		if (query === "") {
			...
			roleDetails.forEach((role) => {
				if (
					role.role_name.toLowerCase().includes(query.toLowerCase())
				) {
	...
	async function handleSkillSort() {
		...
		Object.entries(filtered_staff_skills).forEach(
			([staff_id, staffSkills]) => {
				let skill_id_array: Array<number> = [];
		...
		let sorted_staff_ids: Array<number> = matchSkills
			.sort((a, b) => {
					...
					<Table hoverable className="w-full">
						<Table.Head>
							...
							{selected !== undefined ? (
								...
								>
									Skill Match{" "}
									<FaSort className="inline mb-0.5"></FaSort>
								</Table.HeadCell>
							) : null}
						</Table.Head>
						<Table.Body>
							{staff.map((staff) => {
								let single_staff_skill_ids =
									staffSkills[staff.staff_id];
								let single_staff_skills: Array<TSkillDetails> =
									[];
								if (single_staff_skill_ids) {
									single_staff_skill_ids.forEach(
										(skill_id) => {
											skills.find((skill) => {
												if (
													skill.skill_id === skill_id
												) {
													single_staff_skills.push(
														skill
													);
												}
											});
														...
														if (
															...
															return (
																<Table.Cell>
																	{(staffSkills[1]
																		.length /
																		filteredRoleSkillDetails.length) *
																		100 >
																	100
																		? 100
																		: (staffSkills[1]
