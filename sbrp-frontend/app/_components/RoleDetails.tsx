...
type RoleDetailsProps = {
	...
	roleDetailsToastProps: {
		...
		setShowWithdrawSuccessToast: React.Dispatch<
			React.SetStateAction<boolean>
		>;
		showWithdrawErrorToast: boolean;
		setShowWithdrawErrorToast: React.Dispatch<
			React.SetStateAction<boolean>
		>;
...
export default function RoleDetails({
	...
	const [openSubmitModal, setOpenSubmitModal] = useState<string | undefined>(
		undefined
	);
	...
	async function getRoleListingChanges(): Promise<
		Array<TRoleListingChanges>
	> {
		try {
	...
	async function applyRole(role_listing_id: number, staff_id: number) {
		...
		if (response.status === 201) {
			...
			setTimeout(
				() => roleDetailsToastProps.setShowApplySuccessToast(false),
				10000
			);
			...
			setTimeout(
				() => roleDetailsToastProps.setShowApplyErrorToast(false),
				10000
			);
		}
	}
	...
	async function withdrawRole(roleApplication: TRoleApplicant) {
		...
		if (response.status === 200) {
			...
			setTimeout(
				() => roleDetailsToastProps.setShowWithdrawSuccessToast(false),
				10000
			);
			setAppliedRole(response.data.data);
		} else {
			...
			setTimeout(
				() => roleDetailsToastProps.setShowWithdrawErrorToast(false),
				10000
			);
		}
	}
	...
	async function editListing(editedRoleData: any) {
		...
		if (response.status === 200) {
			...
			setTimeout(
				() => roleDetailsToastProps.setShowEditSuccessToast(false),
				10000
			);
		} else {
			roleDetailsToastProps.setShowEditErrorToast(true);
			setTimeout(
				() => roleDetailsToastProps.setShowEditErrorToast(false),
				10000
			);
		}
	}
												...
												<div className="space-y-4">
													...
													<div className="flex flex-col">
														...
														<textarea
															...
															value={
																editedRoleData.role_listing_desc
															}
															onChange={(e) =>
																setEditedRoleData(
																	{
													...
													<div className="flex flex-col">
														...
														<input
															...
															required
															value={
																editedRoleData.role_listing_open
															}
														/>
													</div>
													{/* Role Close */}
													<div className="flex flex-col">
														...
														<input
															...
															required
															value={
																editedRoleData.role_listing_close
															}
														/>
													</div>
														...
														<Dropdown
															...
															label={
																editedRoleData.role_listing_status
																	.charAt(0)
																	.toUpperCase() +
																editedRoleData.role_listing_status.slice(
																	1
																)
															}
															className="max-h-40"
						...
						<div className="col-span-3">
							<p className="font-normal text-sm text-gray-700 dark:text-white">
								{roleSkillsDetails.map((skill) => {
									if (
										currUserSkills.includes(skill.skill_id)
									) {
										return (
											<span
